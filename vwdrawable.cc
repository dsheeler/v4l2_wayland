...
int vwDrawable::activate_spin(double scale) {
	double duration = 2;
	if (!this->active) {
		this->easers.clear();
		fade_in(duration);
		...
		er2->start();
		Easer *er3 = new Easer();
		er3->initialize(this->dingle_dots, this, EASER_SINE_EASE_OUT, std::bind(&vwDrawable::set_scale, this, std::placeholders::_1), 0, scale, duration);
		er3->start();
		gtk_widget_queue_draw(this->dingle_dots->drawing_area);
		this->active = 1;
	}
